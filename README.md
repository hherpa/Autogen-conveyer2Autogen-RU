...
Например запрос пользователя это сделать полноценную игру на python. Под полноценной я подразумеваю не только python скрипт, но и например звуковое сопровождение, хорошое визуальное оформление, проработанный сюжет и т.д. Очевидно, что обычный групповой чат autogen (всего лишь один отдел) не способен справиться с этой задачей. Полуавтоматический инструмент Conveyer2Conveyer должен быть способен предоставить пользователю примерный список отделов и их цепочку взаимосвязей вплоть до сборки. В примере с полноценной игрой на python, Conveyer2Conveyer должен предложить пользователю подобные отделы:
```
1. Отдел разработки кода
  | ...
2. Отдел разработки музыкального сопровождения
  | интегрируемый инструмент: https://github.com/rushidarge/Music-Generator-Ai or https://github.com/salu133445/musegan
  | ...
3. Отдел разработки интерфейса
  | интегрируемый инструмент: https://github.com/microsoft/autogen/blob/main/notebook/agentchat_dalle_and_gpt4v.ipynb
  | ...
4. Отдел разработки сюжета
  | ...
И т.д.
```
Это всего лишь 4 отдела (не считая отдела сборки) осознание того что под запрос пользователя Conveyer2Conveyer может предложить и 20 и 30 отделов просто поражает.

# Результаты работы
Реализация Conveyer2Autogen и Conveyer2Conveyer может являться большим шагом для autogen, а в особенности для autobuild, но лишь маленьким шагом для IntelliLex. Для нас эти два инструмента являются лишь способами автоматического создания конвейеров/отделов в огромных комплексных системах нейросетей. Да, это огромная оптимизация процессов которая может сократить целые месяцы работы всего лишь до одного вечера. Также это привличение новых пользователей для autogen и в целом популяризация этой технологии так как не нужно писать код, а чтобы получить желаемый результат достаточно написать свой запрос и вставить API ключ. Но LLM стремительно развиваются с каждым днём, но каков смысл если их мощности мы не используем в полную силу? Я бы даже сказал, что почти и не используем так как ста отделовую систему можно построить и на одном mistral-7b в textgen-webui! В наших планах еще много грандиозных проектов на технологии конвейеров/отделов, а поэтому я могу смело заявить, что это большой шаг для autogen и маленький шаг для IntelliLex.

Также не стоит забывать про наши настоятельные предложения по добавлению нового класса агентов в autogen, а также развитию универсального типа агентов, что смогут 100% улучшить работоспособность практически любого чата. Мы должны по максимому использовать мощности LLM и проект IntelliLex готов помочь в этом!
